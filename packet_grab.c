		...
		const u_char* packet)
{
 ...
 #define IP_V(ip)  (((ip)->ip_vhl) >> 4)
	...
	/* Вывод длины пакета и даты*/
	printf("****************************************\n"
			"Recieved Packet Size: %d\n", (int)header->len);
	printf("Recieved at ..... %s",ctime((const time_t*)&header->ts.tv_sec));

	/* Выделение и вывод заголовка Ethernet*/
	...
	/* Выделение и вывод заголовка IP только для type=IP*/
    if (ntohs (ethernet->ether_type) == ETHERTYPE_IP)
    {
    ...
    /* Выделение и вывод заголовка TCP только для type=TCP*/
		...
		printf("Payload:\n");
		for(i=SIZE_ETHERNET + size_ip + size_tcp;i<header->len;i++) {
